...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            #     print(a["bbox"])
                gt_boxes = np.concatenate([a["bbox"]], 0)
            # print(len(gt_annos_part))#7
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric,m]):
                ...
                dt_datas_list_woempty = []
                for i in range(len(dt_datas_list)):
                    if len(dt_datas_list[i]):
                        dt_datas_list_woempty.append(dt_datas_list[i])
                ...
                for j, num_part in enumerate(split_parts):
                    ...
                    dt_datas_part = np.concatenate(dt_datas_list_woempty[idx:idx + num_part], 0)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    overlap_0_7 = np.array([[0.25, 0.25, 0.25, 0.25,0.25, 0.25], [0.5,0.5,0.5,0.5,0.5,0.5],
                            [0.7,0.7,0.7,0.7,0.7,0.7]])
    overlap_0_5 = np.array([[0.25, 0.25, 0.25, 0.25,0.25, 0.25], [0.5,0.5,0.5,0.5,0.5,0.5],
                            [0.7,0.7,0.7,0.7,0.7,0.7]])
    min_overlaps = np.stack([overlap_0_7, overlap_0_5], axis=0)  # [2, 3, 5]
    # min_overlaps=[[0.25],[0.25],[0.25],[0.25],[0.25],[0.25]]
    # min_overlaps=overlap_0_25
    class_to_name = {
        0: 'car',
        1: 'pedestrian',
        2: 'cyclist',
